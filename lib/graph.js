...
function Graph(method, url, postData, callback) {
  ...
  this.options.headers = postData.headers
  this.options.multipart = postData.multipart
...
/**
 * Publish photo to an album
 * access token will be needed for posts
 *
 * var wallPost = { message: 'hello', source: '/path/to/photo' }
 * graph.upload_photo(albumID + '/photos', wallPost, callback);
 *
 * @param {string} url
 * @param {object} postData
 * @param {function} callback
...
exports.upload_photo = function (url, postData, callback) {
  if (typeof url !== 'string') {
    return callback({ message: 'Graph api url must be a string' }, null);
  }
  ...
  postData.multipart = [
    ...
    , body: accessToken
      ...
      'Content-Disposition': 'form-data; name="source"; filename="'+path.basename(postData.source)+'"'
    , 'Content-Type' : mime.lookup(postData.source)
    , body: fs.readFileSync(postData.source)
    }, {
